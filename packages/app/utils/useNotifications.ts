...
export function useNotifications(options: UseNotificationsOptions = {}): UseNotificationsResult {
  ...
  const registerToken = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      // Expo tokens must be registered with platform='expo' to match the database enum
      const { data, error: rpcError } = await supabase.rpc('register_push_token', {
        token_value: tokenToRegister,
        token_platform: 'expo' as 'expo' | 'web',
  ...
  }, [enableEventListeners, handleNotification, handleNotificationResponse])
