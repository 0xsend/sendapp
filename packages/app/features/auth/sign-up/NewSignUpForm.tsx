...
export const NewSignUpForm = () => {
  ...
  const handleSubmit = async ({ name }: z.infer<typeof SendtagSchemaWithoutRestrictions>) => {
    ...
    if (schemaError) {
      form.setError('root', {
        type: 'custom',
        message: schemaError.errors[0]?.message ?? 'Invalid Sendtag',
      })
    ...
    if (checkSendtagAvailabilityError) {
      form.setError('root', {
        type: 'custom',
        message: checkSendtagAvailabilityError.message ?? 'Error checking sendtag availability',
      })
  ...
  return (
    <YStack f={1} jc={'space-around'} ai={'center'} gap={'$3.5'} py={'$3.5'}>
      <FormProvider {...form}>
        ...
        <Paragraph>TODO: create a backup</Paragraph>
      </FormProvider>
    </YStack>
  )
}
