...
export function DistributionsStatCard(cardProps: CardProps) {
	return (
		<Card
			bg="$background"
			f={1}
			$gtMd={{ minWidth: 240, flex: 1, flexBasis: 0 }}
			{...cardProps}
		>
			{cardProps.children}
		</Card>
...
export function DistributionProgressCard(props: CardProps) {
	...
	useEffect(() => {
		...
		const _distributed =
			DISTRIBUTION_INITIAL_POOL_AMOUNT - sendTotalDistPool.value;
		setDistributed(_distributed.toLocaleString());
		setDistributionProgress(
			Number(
				(BigInt(_distributed) * BigInt(100)) / DISTRIBUTION_INITIAL_POOL_AMOUNT,
			),
		);
	}, [sendTotalDistPool]);
...
export function DistributionTimeCard(props: CardProps) {
	...
	const isBeforeQualification =
		hasDistribution && now < distribution.qualification_start;
