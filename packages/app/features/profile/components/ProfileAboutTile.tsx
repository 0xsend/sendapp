...
import { IconX, IconSocial } from 'app/components/icons'
  ...
  return (
    <Fade>
      <YStack w={'100%'} gap={'$4'} pb={'$4'} testID={'profile-about-tile'}>
        ...
        {profile.link_in_bio && profile.link_in_bio.length > 0 && (
          <YStack bg={'$color1'} borderRadius={'$6'} padding={'$5'} gap={'$4'} elevation={'$0.75'}>
            <Paragraph size={'$6'} fontWeight="600">
              Social Links
            </Paragraph>
            <YStack gap="$3">
              {profile.link_in_bio.map(({ domain, handle, domain_name }) => (
                <XStack
                  key={domain_name}
                  ai="center"
                  jc="space-between"
                  ...
                  gap={'$3'}
                  cursor={'pointer'}
                  onPress={() => handleLinkInBioRedirect({ domain, handle })}
                  pressStyle={{ opacity: 0.7 }}
                  hoverStyle={{ opacity: 0.8 }}
                >
                  <XStack gap={'$3'} ai="center" flex={1}>
                    <IconSocial domain_name={domain_name} size={24} color={'$color10'} />
                    <YStack flex={1}>
