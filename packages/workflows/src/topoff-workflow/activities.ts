...
type TopoffActivities = {
  checkEthBalance: (address: Address) => Promise<bigint>
  checkPaymasterDeposit: (address: Address) => Promise<bigint>
  sendEth: (to: Address, amount: bigint) => Promise<Hex>
  sendBundlerSelfTransaction: () => Promise<Hex>
  depositToPaymaster: (address: Address, amount: bigint) => Promise<Hex>
  checkAndTopOffAccount: (config: AccountConfig) => Promise<{
    address: Address
    name: string
  ...
  checkFundingWalletEthBalance: () => Promise<bigint>
  checkUSDCBalance: () => Promise<bigint>
  checkUsdcBalanceOf: (address: Address) => Promise<bigint>
  sendUsdc: (to: Address, amount: bigint) => Promise<Hex>
  calculateTotalETHNeeded: (configs: AccountConfig[]) => Promise<bigint>
  calculateTotalUSDCNeeded: (configs: AccountConfig[]) => Promise<bigint>
  logTopOffSummary: (
    results: Array<{
      address: Address
      name: string
...
export const createTopoffActivities = (
  ...
  function getFundingWallet() {
    const privateKey = env.FUNDING_TOPOFF_PRIVATE_KEY ?? process.env.FUNDING_TOPOFF_PRIVATE_KEY
    if (!privateKey) {
   ...
   */
  async function checkEthBalance(address: Address): Promise<bigint> {
   ...
   */
  async function checkPaymasterDeposit(address: Address): Promise<bigint> {
   ...
   */
  async function sendEth(to: Address, amount: bigint): Promise<Hex> {
   ...
   */
  async function sendBundlerSelfTransaction(): Promise<Hex> {
   ...
   */
  async function depositToPaymaster(address: Address, amount: bigint): Promise<Hex> {
    const account = getFundingWallet()
    const chainId = baseMainnetClient.chain.id
   ...
   */
  async function checkAndTopOffAccount(config: AccountConfig): Promise<{
    address: Address
    name: string
    currentBalance: string
    topped: boolean
  ...
  async function checkFundingWalletEthBalance(): Promise<bigint> {
    const account = getFundingWallet()
  ...
  async function checkUsdcBalanceOf(address: Address): Promise<bigint> {
    const chainId = baseMainnetClient.chain.id
    const balance = await baseMainnetClient.readContract({
      address: usdcAddress[chainId],
      abi: erc20Abi,
  ...
  async function sendUsdc(to: Address, amount: bigint): Promise<Hex> {
    const account = getFundingWallet()
    const chainId = baseMainnetClient.chain.id
  ...
  async function calculateTotalETHNeeded(configs: AccountConfig[]): Promise<bigint> {
    let totalNeeded = 0n
  ...
  async function calculateTotalUSDCNeeded(configs: AccountConfig[]): Promise<bigint> {
    let totalNeeded = 0n
  ...
  async function logTopOffSummary(
    results: Array<{
      address: Address
      name: string
      currentBalance: string
