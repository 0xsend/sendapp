...
export const createUserOpActivities = (
  ...
  return {
    ...
    async sendUserOpActivity(userOp) {
      try {
        const hash = await cdpBundlerClient.sendUserOperation({
    ...
    async waitForTransactionReceiptActivity(hash) {
      try {
        ...
        const bundlerReceipt = await cdpBundlerClient.waitForUserOperationReceipt({ hash: hexHash })
    ...
    async waitForUserOperationReceiptActivity(params: WaitForUserOperationReceiptParameters) {
      try {
        const bundlerReceipt = await cdpBundlerClient.waitForUserOperationReceipt(params)
