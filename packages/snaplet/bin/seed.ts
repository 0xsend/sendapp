...
/**
 * Seeds shovel integration database for /send check events
 * - /send check created events (`send_check_created`)
 * - /send check claimed events (`send_check_claimed`)
 * @param {SeedClient} seedClient
 */
const seedSendCheckEvents = async (seedClient: SeedClient) => {
  // seed `send_check_created` table
  const seedSendCheckCreated = async () => {
    const sendCheckCreatedEvent = {
      from: '0x123',
    }
    await seedClient.sendCheckCreateds(Array(5).fill(sendCheckCreatedEvent))
  }
  await seedSendCheckCreated()

  // seed `send_check_claimed` table
  const seedSendCheckClaimed = async () => {
    const sendCheckClaimedEvent = {
      from: '0x123',
    }
    await seedClient.sendCheckClaimeds(Array(5).fill(sendCheckClaimedEvent))
  }
  await seedSendCheckClaimed()
}
...
;(async () => {
  ...
  await seedSendCheckEvents(seed)
  await pgClient.end()
