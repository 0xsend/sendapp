import { COSE_PUB_KEY_ALG, } from '../src';
...
describe('Webauthn Credential Functions', () => {
    describe('createPublicKeyCredential', () => {
        describe('with mock', () => {
            ...
            const keyPair = {
                publicKey: Buffer.from('3059301306072a8648ce3d020106082a8648ce3d030107034200040ff364eaaac6d529d6d5f197b91b6827278840cd50ffacc5a4c26514f635edc8742deb22ed8c4a336aff7a86d93d6ccb02eb8a85ce2b2a6ea3409b44c574af9a', 'hex'),
...
-----END EC PRIVATE KEY-----`,
            ...
            it('should create and get a credential with mock', async () => {
                const { createPublicKeyCredential, getPublicKeyCredential, deserializePublicKeyCredentialAttestion, deserializePublicKeyCredentialAssertion, } = await import('../src');
                ...
                const verified = crypto.verify('sha256', Buffer.concat([
        ...
        describe('without mock', () => {
            ...
            it('should create a credential', async () => {
                const { createPublicKeyCredential, deserializePublicKeyCredentialAttestion } = await import('../src');
                const attestationChallenge = Buffer.from('test challenge').toString('base64');
                const credOptSer = {
                    publicKey: {
                        rp: {
...
function verifyCredChallenge({ cred, attestationChallenge, publicKey, }) {
    ...
    const verified = crypto.verify('sha256', Buffer.concat([attestation.authData, response.clientDataJSON]), crypto.createPublicKey({
        key: publicKey,
        format: 'der',
        type: 'spki',
