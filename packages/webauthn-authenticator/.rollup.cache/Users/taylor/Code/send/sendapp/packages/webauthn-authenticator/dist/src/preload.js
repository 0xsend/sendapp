 ...
 */
export function installWebAuthnMock({ exposedCreateCredFuncName, exposedGetCredFuncName, } = {}) {
    ...
    navigator.credentials.create = async (credOpt) => {
        ...
        const credOptSer = {
            publicKey: {
                ...credOpt.publicKey,
                challenge: Buffer.from(credOpt.publicKey.challenge).toString('base64'),
                user: {
                    ...credOpt.publicKey.user,
                    id: Buffer.from(credOpt.publicKey.user.id).toString('base64'),
                },
                excludeCredentials: credOpt.publicKey.excludeCredentials?.map((c) => {
                    return {
                        ...c,
                        id: Buffer.from(c.id).toString('base64'),
                    };
                }),
            },
        };
    ...
    navigator.credentials.get = async (credential) => {
        ...
        const credOpts = {
            publicKey: {
                ...
                challenge: Buffer.from(credential.publicKey.challenge).toString('base64'),
