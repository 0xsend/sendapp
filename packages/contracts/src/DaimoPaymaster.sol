...
contract DaimoPaymaster is BasePaymaster {
    ...
    event UserOperationSponsored(bytes32 indexed userOpHash, uint256 requiredPreFund);

    constructor(IEntryPoint _entryPoint, address _owner, IMetaPaymaster _metaPaymaster) BasePaymaster(_entryPoint) {
        transferOwnership(_owner);
        metaPaymaster = _metaPaymaster;
    }

    function setDestAddressWhitelist(address[] calldata addresses, bool isWhitelisted) public onlyOwner {
        ...
        returns (bytes memory context, uint256 validationData)
    {
        ...
        require(recoveredSigner == ticketSigner, "DaimoPaymaster: invalid ticket signature");

        // Additionally, only certain calls are sponsored
        require(userOp.callData.length > 4, "DaimoPaymaster: no callData call");
        bytes4 selector = bytes4(userOp.callData[0:4]);
        require(DaimoAccount.executeBatch.selector == selector, "DaimoPaymaster: callData not for executeBatch");
        DaimoAccount.Call[] memory calls = abi.decode(userOp.callData[4:], (DaimoAccount.Call[]));
