...
contract DaimoEphemeralNotesV2 {
    ...
    function createNote(address _ephemeralOwner, uint256 _amount) external {
        ...
        notes[_ephemeralOwner] = Note({ephemeralOwner: _ephemeralOwner, from: msg.sender, amount: _amount});
    ...
    function claimNoteRecipient(address _ephemeralOwner, address recipient, bytes memory _signature) external {
        Note memory note = notes[_ephemeralOwner];
        require(note.ephemeralOwner != address(0), "EphemeralNotes: note does not exist");

        bytes32 message = ECDSA.toEthSignedMessageHash(keccak256(abi.encodePacked(recipient)));
