...
if CFG.dockerize:
    ...
    inspect_output = str(
        local(
            "docker image inspect {NEXT_COMPOSE_IMAGE} --format='FYSI' 2>/dev/null || echo 'Nope'".format(
                NEXT_COMPOSE_IMAGE=NEXT_COMPOSE_IMAGE
            ),
            quiet=True,
            echo_off=True,
        )
    ...
    if inspect_output != "FYSI":
        print(
            color.yellow(
                "{} does not exist locally, Building docker image".format(
                    NEXT_COMPOSE_IMAGE
                )
            )
        )
    ...
    local_resource(
        "next:web",
        (
            "yarn turbo build --filter=next-app" if CI else ""
        ),  # In CI, only build the web app
        labels=labels,
        ...
        serve_cmd=(
            "" if CI else "yarn next-app dev"
        ),  # In CI, playwright tests start the web server
    )
...
local_resource(
    ...
    serve_cmd="yarn run distributor start" if CI else "yarn run distributor dev",
)
