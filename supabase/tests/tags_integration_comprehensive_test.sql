...
INSERT INTO sendtag_checkout_receipts (
    ...
    decode('0000000000000000000000000000000000000000', 'hex'),  -- log_addr
    EXTRACT(EPOCH FROM NOW())::numeric,  -- block_time
    decode('1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef', 'hex'),  -- tx_hash
...
-- Confirm tag with referral code (using integration_user1's tag as referrer)
SELECT confirm_tags(
    ...
    'junction_tag2'::text  -- referral code (user1's remaining tag)
);
...
-- Test 11: Complex integration scenario - multiple users, tags, and operations
-- User1 creates another tag, user2 tries to claim expired version
SELECT tests.authenticate_as('integration_user1');
SELECT create_tag('complex_tag', (SELECT id FROM send_accounts WHERE user_id = tests.get_supabase_uid('integration_user1')));

-- Expire the tag by backdating it
UPDATE tags
SET created_at = NOW() - INTERVAL '31 minutes'
WHERE name = 'complex_tag';

-- User2 claims the expired tag
SELECT tests.authenticate_as('integration_user2');
...
INSERT INTO sendtag_checkout_receipts (
    ...
    decode('0000000000000000000000000000000000000000', 'hex'),  -- log_addr
    EXTRACT(EPOCH FROM NOW())::numeric,  -- block_time
    decode('789abcdef0123456789abcdef0123456789abcdef0123456789abcdef012345', 'hex'),  -- tx_hash
    decode('2222222222222222222222222222222222222222', 'hex'),  -- sender (matches user2's address)
    1000000000000000000::numeric,  -- amount
    decode('0000000000000000000000000000000000000000', 'hex'),  -- referrer
    0::numeric,  -- reward
