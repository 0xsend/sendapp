...
insert into
    public.send_accounts (user_id, address, chain_id, init_code)
values
    (
    ...
    (
        ...
        floor(
            extract(
                epoch
                from
                    timestamptz '2024-05-26T13:38:25.000000Z'
            )
        ),
        '\x1234',
        '\x1234',
...
-- alice refers bob
insert into
    public.referrals (referrer_id, referred_id, tag, tag_id)
values
    (
        tests.get_supabase_uid('alice'),
        tests.get_supabase_uid('bob'),
...
insert into
    public.sendtag_checkout_receipts (
        "chain_id",
        "log_addr",
        "block_time",
        "tx_hash",
...
select
    results_eq(
        ...
        select
            ...
            rewards_usdc :: int
        from
            public.leaderboard_referrals_all_time()
        where
            ("user").id = tests.get_supabase_uid('alice')
        limit
            1 $$, $$
        values
            (tests.get_supabase_uid('alice'), 1, 100) $$,
            'alice is at the top of the leaderboard'
    );
...
select
    is_empty(
        ...
        from
            public.leaderboard_referrals_all_time()
        where
            ("user").id = tests.get_supabase_uid('bob')
        order by
            referrals desc
        limit
            1 $$, 'bob is at the second position'
    );
...
-- bob refers belle
insert into
    public.tags (name, status, user_id)
values
    (
        'belle',
        'confirmed',
