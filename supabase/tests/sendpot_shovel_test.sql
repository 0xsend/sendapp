...
-- Trigger calculates: tickets = FLOOR(total_bps / net_bps_per_ticket)
INSERT INTO public.sendpot_user_ticket_purchases (
    ...
    -- User A: BPS amounts per transaction
    (1, '\x01', 1000, '\x01', '\xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', 100, '\xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', '\xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', 70000, 'ig1', 'src1', 50, 1, 1, 1),    -- Run 1: 70000 BPS -> 10 tickets
    (1, '\x01', 1100, '\x02', '\xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', 200, '\xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', '\xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', 210000, 'ig1', 'src1', 100, 1, 1, 1),   -- Run 2: 210000 BPS -> 30 tickets
    (1, '\x01', 1100, '\x02', '\xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', 200, '\xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', '\xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', 350000, 'ig1', 'src1', 150, 1, 1, 1),   -- Run 2: 350000 BPS -> 50 tickets (total 80 for run 2)
    -- User B: separate buyer
    (1, '\x01', 1200, '\x03', NULL, 300, '\xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb', '\xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb', 210000, 'ig1', 'src1', 50, 1, 1, 2),    -- Run 1: 210000 BPS -> 30 tickets
    -- User A continues
    (1, '\x01', 2100, '\x04', '\xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', 400, '\xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', '\xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', 630000, 'ig1', 'src1', 200, 1, 1, 1),   -- Run 3: 630000 BPS -> 90 tickets
    -- User B continues
    (1, '\x01', 2200, '\x05', NULL, 500, '\xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb', '\xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb', 350000, 'ig1', 'src1', 250, 1, 1, 1),   -- Run 2: 350000 BPS -> 50 tickets
    -- User A pending
    (1, '\x01', 3100, '\x06', '\xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', 600, '\xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', '\xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', 1050000, 'ig1', 'src1', 310, 1, 1, 1); -- Pending: 1050000 BPS -> 150 tickets
...
-- =============================================================================
-- Verify the trigger correctly calculates tickets from BPS amount using fee history
...
SELECT results_eq(
    ...
    $$ VALUES ('\xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'::bytea, 50::numeric,  70000::numeric, 10::numeric),
              ('\xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'::bytea, 100::numeric, 210000::numeric, 30::numeric),
              ('\xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'::bytea, 150::numeric, 350000::numeric, 50::numeric),
              ('\xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'::bytea, 200::numeric, 630000::numeric, 90::numeric),
              ('\xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'::bytea, 310::numeric, 1050000::numeric, 150::numeric) $$,
    'Trigger: User A tickets_purchased_count calculated correctly from total BPS'
);
...
SELECT results_eq(
    ...
    $$ VALUES ('\xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb'::bytea, 50::numeric, 210000::numeric, 30::numeric),
              ('\xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb'::bytea, 250::numeric, 350000::numeric, 50::numeric) $$,
    'Trigger: User B tickets_purchased_count calculated correctly from total BPS'
);
...
INSERT INTO public.sendpot_user_ticket_purchases (
    ...
    (1, '\x01', 5000, '\x07', NULL, 700, '\xcccccccccccccccccccccccccccccccccccccccc', '\xcccccccccccccccccccccccccccccccccccccccc', 9000, 'ig1', 'src1', 39000000, 1, 1, 1),   -- 9000 BPS -> 3 tickets (9000/3000)
    (1, '\x01', 5100, '\x08', NULL, 800, '\xcccccccccccccccccccccccccccccccccccccccc', '\xcccccccccccccccccccccccccccccccccccccccc', 21000, 'ig1', 'src1', 39100000, 1, 1, 1);  -- 21000 BPS -> 7 tickets (21000/3000)
...
SELECT results_eq(
    ...
    $$ VALUES ('\xcccccccccccccccccccccccccccccccccccccccc'::bytea, 39000000::numeric, 9000::numeric, 3::numeric),
              ('\xcccccccccccccccccccccccccccccccccccccccc'::bytea, 39100000::numeric, 21000::numeric, 7::numeric) $$,
...
-- Run 2 (block 200): purchases at blocks 100, 150 -> 30 + 50 = 80 tickets
-- Run 3 (block 300): purchases at block 200 -> 90 tickets
SELECT results_eq(
    ...
    $$ VALUES (3, 300::numeric, 90::numeric),
              (2, 200::numeric, 80::numeric),
...
-- Purchases after last jackpot (block 300): block 310 -> 150 tickets
SELECT results_eq(
    $$ SELECT public.get_pending_jackpot_tickets_purchased() $$,
    $$ VALUES (150::numeric) $$,
    'Pending Tickets Function: User A should have 150 tickets pending after Run 3 (block 300)'
);
