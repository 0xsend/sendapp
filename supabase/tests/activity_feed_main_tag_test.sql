...
-- Test 1: Check that user 1 sees correct main tags
SELECT tests.authenticate_as('test_user_1');
SELECT results_eq(
    $$
        SELECT 
    ...
    $$,
    $$
        VALUES ('beta'::text, 'zeta'::text)
    $$,
    'User 1 should see beta as their main tag and zeta as user 2 main tag'
);

-- Test 2: Check that user 2 sees correct main tags
SELECT tests.authenticate_as('test_user_2');
SELECT results_eq(
    $$
        SELECT 
    ...
    $$
        VALUES ('beta'::text, 'zeta'::text)
    $$,
    'User 2 should see beta as user 1 main tag and zeta as their main tag'
);
