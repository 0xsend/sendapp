...
BEGIN
    ...
    SELECT create_tag('lifecycletag1', test_send_account_id) INTO tag_id;
    
    PERFORM ok(tag_id IS NOT NULL, 'create_tag returns tag_id');
    PERFORM ok(EXISTS(
    ...
    UPDATE tags SET status = 'available', user_id = NULL WHERE id = tag_id;
