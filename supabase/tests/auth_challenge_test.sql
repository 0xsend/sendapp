...
-- Use service role to nupass RLS (required to challenges table)
...
SELECT throws_ok(
    ...
    $$,
    'permission denied for table challenges',
...
-- Try executing upsert_challenges
SELECT throws_ok(
    $$
        SELECT "public"."upsert_challenges"(
            tests.get_supabase_uid('auth_user'),
            '4bb9d0e3425f19f33db4b5e8b5f2a5c5326de94330bf9ea4750a3c8604a6383e'
        )::challenges;
    $$,
    'permission denied for table challenges',
    'User should not be able to call upsert_challenges directly.'
);
...
SELECT lives_ok(
    $$
        SELECT (user_id, challenge) FROM challenges
        WHERE user_id = tests.get_supabase_uid('auth_user');
    $$,
