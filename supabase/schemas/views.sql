    ...
    ( SELECT "json_agg"("row_to_json"("t".*)) AS "json_agg"
           FROM ( WITH "new_affiliates" AS (
                 ...
                 SELECT "na"."user_id",
                    ...
                    "p"."avatar_url",
                    "p"."name",
                    "coalesce"("t"."name", "p"."send_id") AS "tag",
                    "na"."affiliate_created_at",
                    "na"."referral_count"
                   FROM (("new_affiliates" "na"
                     LEFT JOIN "public"."profiles" "p" ON (("p"."id" = "na"."user_id")))
                     LEFT JOIN "public"."tags" "t" ON ((("t"."user_id" = "na"."user_id") AND ("t"."status" = 'confirmed'::"public"."tag_status"))))) "t") AS "new_affiliates",
    ( SELECT "json_agg"("row_to_json"("t".*)) AS "json_agg"
           FROM ( SELECT "send_account_credentials"."credential_id",
                    "p"."id",
                    "p"."avatar_url",
                    "p"."name",
                    "t"."name" AS "tag",
                    "send_account_credentials"."created_at"
                   FROM (("public"."send_account_credentials"
                     LEFT JOIN "public"."profiles" "p" ON (("p"."id" = ( SELECT "send_accounts"."user_id"
                           FROM "public"."send_accounts"
                          WHERE ("send_accounts"."id" = "send_account_credentials"."account_id")))))
                     LEFT JOIN "public"."tags" "t" ON ((("t"."user_id" = "p"."id") AND ("t"."status" = 'confirmed'::"public"."tag_status"))))
                  ORDER BY "send_account_credentials"."created_at" DESC
                 LIMIT 10) "t") AS "new_passkeys",
    ( SELECT "json_agg"("row_to_json"("t".*)) AS "json_agg"
           FROM ( SELECT "top_all_ips"."ip_address",
                    "top_all_ips"."transfer_data",
                    "top_all_ips"."tx_count"
                   FROM "top_all_ips"
                 LIMIT 10) "t") AS "top_ips";
