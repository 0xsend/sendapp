...
create view dashboard_metrics
            (passkeys, sendtags, daily_active_senders, daily_active_receivers, daily_active_transfers,
             total_transactions, usdc_volume, send_volume, sendtag_revenue, sendtag_referral_payouts, new_sendtags,
             new_affiliates, top_all_ips)
as
WITH time_window AS (SELECT EXTRACT(epoch FROM now() - '24:00:00'::interval) AS cutoff_time),
     daily_transfers AS (SELECT t.f,
                                t.t,
                                t.log_addr,
                                t.v,
                                t.block_time
     ...
     recent_transfers AS (SELECT t.f     AS from_addr,
                                 t.t     AS to_addr,
                                 t.log_addr,
                                 t.v     AS amount,
                                 t.block_time,
                                 CASE
                                     WHEN t.log_addr =
                                          decode('833589fcd6edb6e08f4c7c32d4f71b54bda02913'::text, 'hex'::text)
                                         THEN t.v / 1000000.0
                                     ELSE 0::numeric
                                     END AS usdc_amount,
                                 CASE
                                     WHEN t.log_addr = ANY
                                          (ARRAY [decode('3f14920c99beb920afa163031c4e47a3e03b3e4a'::text,
                                                         'hex'::text), decode(
                                                  'Eab49138BA2Ea6dd776220fE26b7b8E446638956'::text, 'hex'::text)])
                                         THEN t.v / 1000000000000000000.0
                                     ELSE 0::numeric
                                     END AS send_amount
                          FROM daily_transfers t),
     account_mapping AS (SELECT rt.from_addr,
                                rt.to_addr,
                                rt.log_addr,
                                rt.amount,
                                rt.block_time,
                         ...
                         FROM recent_transfers rt
                                  LEFT JOIN send_accounts sa_from
                                            ON lower(concat('0x', encode(rt.from_addr, 'hex'::text)))::citext =
                                               sa_from.address
                                  LEFT JOIN profiles p_from ON p_from.id = sa_from.user_id
                                  LEFT JOIN send_accounts sa_to
                                            ON lower(concat('0x', encode(rt.to_addr, 'hex'::text)))::citext =
                                               sa_to.address
                                  LEFT JOIN profiles p_to ON p_to.id = sa_to.user_id),
     ip_transfer_data AS (SELECT s.ip              AS ip_address,
                                 r.ip              AS to_ip,
                                 CASE
                                     WHEN am.log_addr =
                                          decode('833589fcd6edb6e08f4c7c32d4f71b54bda02913'::text, 'hex'::text)
                                         THEN 'USDC'::text
                                     WHEN am.log_addr = ANY
                                          (ARRAY [decode('3f14920c99beb920afa163031c4e47a3e03b3e4a'::text,
                                                         'hex'::text), decode(
                                                  'Eab49138BA2Ea6dd776220fE26b7b8E446638956'::text, 'hex'::text)])
                                         THEN 'SEND'::text
                                                       ...
                                                       WHERE sessions.user_id = ((SELECT profiles.id
                                                                                  ...
                                                                                  WHERE profiles.id =
                                                                                        ((SELECT send_accounts.user_id
                                                                                          FROM send_accounts
                                                                                          WHERE send_accounts.address =
                                                                                                lower(concat('0x', encode(am.from_addr, 'hex'::text)))::citext))))
                                                         AND sessions.created_at <= to_timestamp(am.block_time::double precision)
                                                       ...
                                                       WHERE sessions.user_id = ((SELECT profiles.id
                                                                                  ...
                                                                                  WHERE profiles.id =
                                                                                        ((SELECT send_accounts.user_id
                                                                                          FROM send_accounts
                                                                                          WHERE send_accounts.address =
                                                                                                lower(concat('0x', encode(am.to_addr, 'hex'::text)))::citext))))
                                                         AND sessions.created_at <= to_timestamp(am.block_time::double precision)
                                                         AND sessions.created_at >=
                                                             to_timestamp((am.block_time - 86400::numeric)::double precision)
                                                       ORDER BY sessions.created_at DESC
                                                       LIMIT 1) r ON true
                          WHERE s.ip IS NOT NULL
                          GROUP BY s.ip, r.ip, am.log_addr),
     top_all_ips AS (SELECT ip_transfer_data.ip_address,
                            json_agg(json_build_object('to_ip', ip_transfer_data.to_ip, 'currency',
                                                       ip_transfer_data.currency, 'amount',
                                                       ip_transfer_data.amount)) AS transfer_data,
...
SELECT (SELECT count(DISTINCT send_account_credentials.account_id)::integer AS count
       ...
       (SELECT COALESCE(sum(
                                CASE
                                    WHEN daily_transfers.log_addr =
                                         decode('833589fcd6edb6e08f4c7c32d4f71b54bda02913'::text, 'hex'::text)
                                        THEN COALESCE(daily_transfers.v, 0::numeric) / 1000000.0
                                    ELSE 0::numeric
                                    END), 0::numeric) AS "coalesce"
        FROM daily_transfers)                              AS usdc_volume,
       (SELECT COALESCE(sum(
                                CASE
                                    WHEN daily_transfers.log_addr = ANY
                                         (ARRAY [decode('3f14920c99beb920afa163031c4e47a3e03b3e4a'::text,
                                                        'hex'::text), decode(
                                                 'Eab49138BA2Ea6dd776220fE26b7b8E446638956'::text, 'hex'::text)])
                                        THEN COALESCE(daily_transfers.v, 0::numeric) / 1000000000000000000.0
                                    ELSE 0::numeric
                                    END), 0::numeric) AS "coalesce"
        FROM daily_transfers)                              AS send_volume,
       (SELECT COALESCE(sum(sendtag_checkout_receipts.amount) / 1000000.0, 0::numeric) -
               COALESCE(sum(sendtag_checkout_receipts.reward) / 1000000.0, 0::numeric)
       ...
       (SELECT json_agg(row_to_json(t.*)) AS json_agg
        FROM (WITH new_affiliates AS (SELECT a.user_id,
                   ...
                   recent_transfers AS (SELECT concat('0x', encode(st.t, 'hex'::text))::citext AS receiver_address,
                                               st.v                                            AS amount,
                                               st.block_time
