...
-- Allow inserts only if user_id matches for backward compatibility with tests
-- In production, inserts should go through create_tag function
...
USING (
    -- Users can always see confirmed tags
    status = 'confirmed'::public.tag_status
    -- Or their own tags via send_account_tags
    OR EXISTS (
        SELECT 1
        FROM send_account_tags sat
        JOIN send_accounts sa ON sa.id = sat.send_account_id
    ...
    -- Or if they are service role
    OR current_setting('role')::text = 'service_role'
);

-- Allow users to update their own pending tags
CREATE POLICY "update_policy" ON "public"."tags" FOR UPDATE
USING (
    status = 'pending'::public.tag_status
    AND EXISTS (
        SELECT 1
