        ...
        UNION ALL
         ...
         SELECT scc.sender AS f,
          ...
          WHERE ((scc.block_time >= bounds.min_start) AND (scc.block_time <= bounds.max_end))
            ...
            AND (sa_redeemer.is_verified = true)
...
create or replace view "public"."send_scores_current" as  WITH dws AS (
         ...
         FROM (
           ...
           UNION ALL
           -- Send Check claims senders (SEND token only, verified redeemer)
           -- OPTIMIZED: Direct JOIN with send_accounts for is_verified check
           SELECT scc.sender AS f
           FROM send_check_claimed scc
           JOIN send_accounts sa_redeemer ON scc.redeemer = sa_redeemer.address_bytes
           CROSS JOIN dws dws_1
...
AND stv.f IN (SELECT address_bytes FROM sender_accounts)
             ...
             UNION ALL
             -- Send Check claims (SEND token only, verified redeemer)
             -- OPTIMIZED: Direct JOIN for is_verified check
             SELECT scc.sender AS f,
                    scc.redeemer AS t,
                    scc.amount AS v
               FROM send_check_claimed scc
