...
using (((EXISTS ( SELECT 1
   FROM send_accounts
  WHERE ((send_accounts.user_id = ( SELECT auth.uid() AS uid)) AND ((lower(('0x'::text || encode(send_token_transfers.f, 'hex'::text))))::citext = send_accounts.address)))) OR (EXISTS ( SELECT 1
   FROM send_accounts
  WHERE ((send_accounts.user_id = ( SELECT auth.uid() AS uid)) AND ((lower(('0x'::text || encode(send_token_transfers.t, 'hex'::text))))::citext = send_accounts.address))))));
...
using (((EXISTS ( SELECT 1
   FROM send_accounts
  WHERE ((send_accounts.user_id = ( SELECT auth.uid() AS uid)) AND ((lower(('0x'::text || encode(send_token_v0_transfers.f, 'hex'::text))))::citext = send_accounts.address)))) OR (EXISTS ( SELECT 1
   FROM send_accounts
  WHERE ((send_accounts.user_id = ( SELECT auth.uid() AS uid)) AND ((lower(('0x'::text || encode(send_token_v0_transfers.t, 'hex'::text))))::citext = send_accounts.address))))));
