...
-- Update the initial insert to include sent_to array
...
send_ceiling_settings AS(
  ...
  FROM
    distributions d
    JOIN previous_distribution pd ON TRUE
...
qualifying_sends AS(
  ...
  FROM
    ...
    LEFT JOIN LATERAL sum_qualification_sends($1, scs.send_ceiling) qs ON qs.user_id = scs.user_id)
...
(
  ...
  FROM
    qualifying_sends qs
    JOIN send_ceiling_settings scs ON scs.user_id = qs.user_id
  WHERE
    qs.amount > 0;
END;
$$;
...
BEGIN
  ...
  IF _user_id IS NOT NULL AND _distribution_id IS NOT NULL THEN
    ...
    IF NOT _verification_exists THEN
      -- Get send ceiling from send_ceiling_settings calculation
      WITH send_settings AS (
        SELECT
          minimum_sends * scaling_divisor AS divider
        FROM
...
distribution_info AS (
  ...
  FROM
    distributions
  WHERE
    id = _distribution_id) -- Removed semicolon here
  SELECT
