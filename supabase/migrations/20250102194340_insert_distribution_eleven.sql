-- Update distribution 10
UPDATE
  public.distributions
SET
  (amount,
    description) =(500000000,
    'Tenth distribution of 500,000,000 SEND tokens to early hodlers')
WHERE
  number = 10;
...
-- Round #11
-- 300m $send
-- Opens Jan 01st 00:00 UTC
-- Closes Jan 31st 23:59 UTC
-- 750k minimum
...
VALUES (
  11,
  'Distribution #11',
  'Eleventh distribution of 300,000,000 SEND tokens to early hodlers',
  300000000, -- 300,000,000 SEND
  10000,
  0,
  10000,
  '2025-01-01T00:00:00Z' ::timestamp with time zone,
  '2025-01-31T00:00:00Z' ::timestamp with time zone - interval '1 second',
  750000, -- 750,000 SEND
  'infinity',
  8453 -- Base chain
);
...
WHERE
  distribution_id =(
    ...
    WHERE
      number = 10)
  AND type IN ('tag_registration', 'create_passkey', 'tag_referral', 'total_tag_referrals', 'send_ceiling', 'send_streak');
...
LEAST(sa.created_at,(
    ...
    FROM distributions
    WHERE
      "number" = 1 LIMIT 1)) AS created_at
FROM
  send_accounts sa
WHERE
  sa.created_at <=(
...
SELECT
  calculate_and_insert_send_ceiling_verification(11);
