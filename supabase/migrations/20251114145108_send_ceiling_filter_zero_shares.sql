...
CREATE OR REPLACE FUNCTION public.insert_tag_referral_verifications(distribution_num integer)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
DECLARE
...
CREATE OR REPLACE FUNCTION public.insert_total_referral_verifications(distribution_num integer)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
DECLARE
           ...
           FROM ( SELECT stt.f
                ...
                UNION ALL
                 SELECT stv.f
                   FROM (send_token_v0_transfers stv
                     CROSS JOIN dws dws_1)
                  WHERE ((stv.block_time >= dws_1.start_time) AND (stv.block_time <= dws_1.end_time))) all_senders
