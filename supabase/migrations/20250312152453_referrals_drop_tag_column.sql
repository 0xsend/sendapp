-- drop tag on referrals since it's users should only every have one referrer
-- first drop not null constraint on tag
alter table referrals
  alter column tag drop not null;

-- update confirm_tags to not set tag on referrals
...
-- now drop the tag column from referrals
alter table referrals
  drop column tag;
