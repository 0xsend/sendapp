...
alter table "public"."bridge_deposits" add constraint "bridge_deposits_status_check" CHECK ((status = ANY (ARRAY['funds_received'::text, 'in_review'::text, 'payment_submitted'::text, 'payment_processed'::text, 'refund'::text]))) not valid;
