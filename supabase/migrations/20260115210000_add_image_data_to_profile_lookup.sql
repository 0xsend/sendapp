-- Add avatar_data and banner_data to profile_lookup_result type

-- no-grants: grants inherited from previous referrer view definition
drop view if exists "public"."referrer";

set check_function_bodies = off;

alter type "public"."profile_lookup_result" add attribute "avatar_data" jsonb;
alter type "public"."profile_lookup_result" add attribute "banner_data" jsonb;
...
create or replace view "public"."referrer" with (security_invoker=on, security_barrier=on) as  WITH referrer AS (
         SELECT p.send_id
           FROM (referrals r
         ...
         SELECT p.id,
           ...
           FROM (profile_lookup('sendid'::lookup_type_enum, ( SELECT (referrer_1.send_id)::text AS send_id
                   FROM referrer referrer_1)) p(id, avatar_url, name, about, refcode, x_username, birthday, tag, address, chain_id, is_public, sendid, all_tags, main_tag_id, main_tag_name, links_in_bio, banner_url, is_verified, avatar_data, banner_data)
 ...
 SELECT profile_lookup.id,
    ...
    profile_lookup.avatar_data,
    profile_lookup.banner_data
   FROM profile_lookup;
