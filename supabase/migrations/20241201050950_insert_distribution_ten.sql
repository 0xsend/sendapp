-- Round #10
-- 300m $send
-- Opens Dec 01st 00:00 UTC
-- Closes Dec 31st 23:59 UTC
-- 400k minimum
...
VALUES (
  10,
  'Distribution #10',
  'Tenth distribution of 400,000,000 SEND tokens to early hodlers',
  400000000, -- 400,000,000 SEND
  10000,
  0,
  10000,
  '2024-12-01T00:00:00Z' ::timestamp with time zone,
  '2025-01-01T00:00:00Z' ::timestamp with time zone - interval '1 second',
  300000, -- 300,000 SEND
  'infinity',
  8453 -- Base chain
);
...
WHERE
  distribution_id =(
    ...
    WHERE
      number = 9)
  AND type IN ('tag_registration', 'create_passkey', 'send_ten', 'send_one_hundred', 'tag_referral', 'total_tag_referrals', 'send_ceiling');
...
LEAST(sa.created_at,(
    ...
    FROM distributions
    WHERE
      "number" = 10 LIMIT 1)) AS created_at
FROM
  send_accounts sa
WHERE
  sa.created_at <=(
...
SELECT
  calculate_and_insert_send_ceiling_verification(10);
