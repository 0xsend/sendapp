-- update amount to match amount_after_slash for all distributions
UPDATE
  public.distribution_shares
SET
  amount = amount_after_slash;
...
FROM
  ...
  VALUES (
    ...
    jsonb_build_object(
      'value', _send_ceiling::text, 'sent_to', ARRAY[_recipient_address] ::citext[]));
ELSE
  ...
  SET
    -- Cast to text to avoid overflow errors on client
    metadata = jsonb_build_object('value', _send_ceiling::text, 'sent_to', metadata -> 'sent_to' || jsonb_build_array(_recipient_address)),
