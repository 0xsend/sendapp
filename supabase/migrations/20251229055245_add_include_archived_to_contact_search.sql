drop function if exists "public"."contact_search"(p_query text, p_limit_val integer, p_offset_val integer, p_favorites_only boolean, p_label_ids bigint[], p_source_filter contact_source_enum[]);

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.contact_search(p_query text DEFAULT NULL::text, p_limit_val integer DEFAULT 50, p_offset_val integer DEFAULT 0, p_favorites_only boolean DEFAULT false, p_label_ids bigint[] DEFAULT NULL::bigint[], p_source_filter contact_source_enum[] DEFAULT NULL::contact_source_enum[], p_include_archived boolean DEFAULT false)
...
BEGIN
    ...
    WHERE c.owner_id = current_uid
      -- Archived filter: show only archived when p_include_archived is true, otherwise exclude archived
      AND (
