...
alter table "public"."send_check_claimed" add column "amount" numeric;

alter table "public"."send_check_claimed" add column "token" bytea;

alter table "public"."send_check_created" add column "amount" numeric;

alter table "public"."send_check_created" add column "token" bytea;

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.get_user_active_checks(user_address bytea)
 RETURNS TABLE(id integer, chain_id numeric, block_time numeric, tx_hash bytea, ephemeral_address bytea, sender bytea, token bytea, amount numeric, expires_at numeric, block_num numeric, abi_idx smallint, is_expired boolean)
 LANGUAGE sql
 STABLE
AS $function$
SELECT
...
CREATE OR REPLACE FUNCTION public.get_user_checks_history(user_address bytea)
 RETURNS TABLE(id integer, chain_id numeric, block_time numeric, tx_hash bytea, ephemeral_address bytea, sender bytea, token bytea, amount numeric, expires_at numeric, block_num numeric, abi_idx smallint, is_claimed boolean, claimed_by bytea, claimed_at numeric)
 LANGUAGE sql
 STABLE
AS $function$
SELECT
