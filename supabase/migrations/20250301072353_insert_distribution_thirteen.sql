-- Round #13
-- 3m $send
-- Opens Mar 01st 00:00 UTC
-- Closes Mar 31st 23:59 UTC
-- 7.5k minimum
...
VALUES (
  13,
  'Distribution #13',
  'Thirteenth distribution of 3,000,000 SEND tokens to early hodlers',
  3000000000000000000000000, -- 3,000,000 SEND
  10000,
  0,
  10000,
  '2025-03-01T00:00:00Z' ::timestamp with time zone,
  '2025-04-01T00:00:00Z' ::timestamp with time zone - interval '1 second',
...
LEAST(sa.created_at,(
    ...
    FROM distributions
    WHERE
      "number" = 13 LIMIT 1)) AS created_at
FROM
  send_accounts sa
WHERE
  sa.created_at <=(
...
SELECT
  calculate_and_insert_send_ceiling_verification(13);
