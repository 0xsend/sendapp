...
CREATE OR REPLACE FUNCTION public.compute_active_distribution_shares(mode text DEFAULT 'ease_in_out'::text)
 RETURNS TABLE(distribution_id integer, user_id uuid, address citext, hodler_pool_amount numeric, fixed_pool_amount numeric, bonus_pool_amount numeric, amount numeric)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.cubic_bezier_multiplier(rank integer, total integer, p1 numeric DEFAULT 0.10, p2 numeric DEFAULT 0.90)
 RETURNS numeric
 LANGUAGE plpgsql
 STABLE
AS $function$
...
create or replace view "public"."user_active_distribution_share" as  SELECT s.distribution_id,
    s.user_id,
    s.address,
    s.hodler_pool_amount,
    s.fixed_pool_amount,
    s.bonus_pool_amount,
    s.amount
   FROM compute_active_distribution_shares('ease_in_out'::text) s(distribution_id, user_id, address, hodler_pool_amount, fixed_pool_amount, bonus_pool_amount, amount)
  WHERE (s.user_id = auth.uid());


create or replace view "public"."active_distribution_shares" as  SELECT compute_active_distribution_shares.distribution_id,
    compute_active_distribution_shares.user_id,
    compute_active_distribution_shares.address,
    compute_active_distribution_shares.hodler_pool_amount,
    compute_active_distribution_shares.fixed_pool_amount,
    compute_active_distribution_shares.bonus_pool_amount,
    compute_active_distribution_shares.amount
   FROM compute_active_distribution_shares('ease_in_out'::text) compute_active_distribution_shares(distribution_id, user_id, address, hodler_pool_amount, fixed_pool_amount, bonus_pool_amount, amount);

-- Privileges and ownership per schema file for live shares
